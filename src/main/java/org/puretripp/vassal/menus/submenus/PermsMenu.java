...
public class PermsMenu <T extends Permissable> extends Menu {
    private T selected;
    private Nation nation;
    private VassalsPlayer target;

    public PermsMenu(boolean isNation, Nation nation, T selected, VassalsPlayer target) {
    ...
    public void populateItems() {
        ...
        ItemStack list = Menu.generateCustomSkull(ChatColor.GREEN + "View Rank", "createMenu",
    ...
    private class PlayerList <T extends Permissable> extends Menu {
        private T selected;
        private VassalsPlayer target;
        private NamespacedKey key = new NamespacedKey(Main.getPlugin(Main.class), "iconClickFunction");

        public PlayerList(boolean isNation, T selected) {
        ...
        public void populateItems() {
            for(UUID uuid : selected.getPlayersUUID()) {
    ...
    private class RankCreation<T extends Permissable> extends Menu {
        private PermClass perm;

        public RankCreation(T obj, VassalsPlayer target) {
            super("Permissions Menu");
            populateItems();
        }
    }
